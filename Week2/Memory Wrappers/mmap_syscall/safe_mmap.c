    ...
    if (addr == MAP_FAILED) {
        switch (errno) {
            ...
            case ENXIO:
                ...
                break;
            case EPERM:
                fprintf(stderr, "mmap failed: Operation not permitted (check security policies)\n");
                break;
            case EFAULT:
                fprintf(stderr, "mmap failed: Bad address (check addr parameter)\n");
                break;
            case EOVERFLOW:
                fprintf(stderr, "mmap failed: Offset or length exceeds file size or addressable range\n");
                break;
            default:
                fprintf(stderr, "mmap failed: Unknown error (errno: %d)\n", errno);
                break;
        }
        return NULL;
    }
